...
>
> **행정동/읍·면 개별 페이지**: 시·구 페이지의 행정동·읍·면 칩은 개별 페이지로 연결(클릭 가능)됩니다. 이 페이지들은 탐색·안내용이며 **`noindex,follow`**(검색 비색인)로 두어 도어웨이(대량 저품질 색인 페이지) 위험을 피합니다. 색인 대표는 상위 시·구/생활권 페이지가 담당합니다. URL 슬러그는 `lib/roman.mjs`(한글→로마자)로 자동 생성됩니다.
